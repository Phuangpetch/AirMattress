...
#define UNO_EVENT_DEDUPE_MS      700UL
...
// UNO duplicate event guard
String lastUnoEventSource = "";
String lastUnoEvent = "";
String lastUnoEventMode = "";
unsigned long lastUnoEventMs = 0;
...
bool isDuplicateUnoEvent(const String &src, const String &evt, const String &mode)
{
  if (src != SRC_KEYPAD) return false;

  unsigned long now = millis();
  if (src == lastUnoEventSource &&
      evt == lastUnoEvent &&
      mode == lastUnoEventMode &&
      (now - lastUnoEventMs) < UNO_EVENT_DEDUPE_MS)
  {
    return true;
  }

  lastUnoEventSource = src;
  lastUnoEvent = evt;
  lastUnoEventMode = mode;
  lastUnoEventMs = now;
  return false;
}
...
void processStateMessage(const String &msg)
{
  ...
  bool duplicateUnoEvent = false;
  if (stateChanged)
  {
    duplicateUnoEvent = isDuplicateUnoEvent(stateSource, stateEvent, stateMode);
    if (!duplicateUnoEvent)
    {
      updateBlynkUI();
  ...
  if (stateChanged && !duplicateUnoEvent && shouldLogSource(stateSource, stateEvent) && !isRecentBlynkEcho(stateSource, stateEvent))
  {
