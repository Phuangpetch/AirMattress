...
#define KEYPAD_RETRY_GAP_MS   40UL
#define AUTO_WAIT_MS        7200000UL
#define RUN_P1_MS           690000UL   // 11 min 30 sec
#define RUN_P2_MS           650000UL   // 10 min 50 sec

#define ALT_SLOT_MS         90000UL    // 1 min 30 sec
...
struct AltRunStatus
{
  bool p1On;
  bool p2On;
  unsigned long p1UsedMs;
  unsigned long p2UsedMs;
  unsigned long totalUsedMs;
  unsigned long totalSessionMs;
};
...
AltRunStatus calcAltRunStatus(unsigned long elapsedMs)
{
  AltRunStatus s = {false, false, 0UL, 0UL, 0UL, 0UL};
  unsigned long rem1 = RUN_P1_MS;
  unsigned long rem2 = RUN_P2_MS;
  bool p1Turn = true;

  while (rem1 > 0 || rem2 > 0)
  {
    if (p1Turn && rem1 == 0UL) { p1Turn = false; continue; }
    if (!p1Turn && rem2 == 0UL) { p1Turn = true; continue; }

    unsigned long sliceMs = p1Turn ? ((rem1 > ALT_SLOT_MS) ? ALT_SLOT_MS : rem1)
                                   : ((rem2 > ALT_SLOT_MS) ? ALT_SLOT_MS : rem2);

    if (elapsedMs < s.totalSessionMs + sliceMs)
    {
      unsigned long inSlice = elapsedMs - s.totalSessionMs;
      s.totalUsedMs = elapsedMs;
      if (p1Turn)
      {
        s.p1UsedMs += inSlice;
        s.p1On = true;
      ...
      s.totalSessionMs += sliceMs;
      return s;
    }

    s.totalSessionMs += sliceMs;
    s.totalUsedMs = s.totalSessionMs;
    if (p1Turn)
    {
      s.p1UsedMs += sliceMs;
      rem1 -= sliceMs;
...
unsigned long totalAltSessionMs()
{
  AltRunStatus s = calcAltRunStatus(0xFFFFFFFFUL);
  return s.totalSessionMs;
}
...
bool pump1On()
{
  ...
  if (currentState == ST_AUTO_RUN || currentState == ST_TIMER_RUN)
  {
    return calcAltRunStatus(millis() - stateStartMs).p1On;
  }
  return singleP1;
}
...
bool pump2On()
{
  ...
  if (currentState == ST_AUTO_RUN || currentState == ST_TIMER_RUN)
  {
    return calcAltRunStatus(millis() - stateStartMs).p2On;
  }
  return singleP2;
}
...
byte normalizeHour(byte v)
{
  return (v >= 24) ? 0 : v;
}

byte normalizeMinuteSecond(byte v)
{
  return (v >= 60) ? 0 : v;
}
...
{
  byte hh = (setDigits[0] - '0') * 10 + (setDigits[1] - '0');
  byte mm = (setDigits[2] - '0') * 10 + (setDigits[3] - '0');
  byte ss = (setDigits[4] - '0') * 10 + (setDigits[5] - '0');

  targetHour   = normalizeHour(hh);
  targetMinute = normalizeMinuteSecond(mm);
  targetSecond = normalizeMinuteSecond(ss);
  targetValid = true;
}
...
void sendKeypadStateReliable(const char *evt)
{
  sendState(SRC_KEYPAD, evt);
  delay(KEYPAD_RETRY_GAP_MS);
  sendState(SRC_KEYPAD, evt);
}
...
void buildPumpLeftLine(char *buf, byte pumpNo, unsigned long runMs)
{
  ...
  AltRunStatus s = calcAltRunStatus(elapsedMs);
  unsigned long usedMs = (pumpNo == 1) ? s.p1UsedMs : s.p2UsedMs;

  if (usedMs >= runMs)
  {
    snprintf(buf, 17, "P%u DONE 00:00", pumpNo);
    return;
  }

  unsigned long remainMs = runMs - usedMs;
  unsigned long totalSec = remainMs / 1000UL;
  unsigned int mm = totalSec / 60UL;
  unsigned int ss = totalSec % 60UL;

  snprintf(buf, 17, "P%u LEFT %02u:%02u", pumpNo, mm, ss);
}
...
void updateLcdCache()
{
  ...
  else if (currentState == ST_SET_TIME)
  {
    ...
    if (setLen >= 6)
    {
      centerTo16(buf1, "PRESS # CONFIRM");
      centerTo16(buf2, t);
    }
    else
    {
      centerTo16(buf1, "ENTER STARTTIME");
      centerTo16(buf2, t);
    }
  }
...
void keypadStopNow()
{
  ...
  lastLcdSwitchMs = millis();
  applyPumps();
  sendKeypadStateReliable("STOP_KEY");
}
...
void pauseOn(const char *src)
{
  ...
  applyPumps();
  if (strcmp(src, SRC_KEYPAD) == 0) sendKeypadStateReliable(EVT_PAUSE_ON);
  else sendState(src, EVT_PAUSE_ON);
}
...
void pauseOff(const char *src)
{
  ...
  applyPumps();
  if (strcmp(src, SRC_KEYPAD) == 0) sendKeypadStateReliable(EVT_PAUSE_OFF);
  else sendState(src, EVT_PAUSE_OFF);
}
...
void handleHash()
{
  ...
  if (currentState == ST_SET_TIME)
  {
    ...
    sendKeypadStateReliable(EVT_TIMER_SET);
    return;
  }
  ...
  if (currentState == ST_AUTO_CONFIRM)
  {
    ...
    sendKeypadStateReliable(EVT_AUTO_ON);
    return;
  }
}
...
void handleB()
{
  ...
  if (currentState == ST_HOME)
  {
    ...
    sendKeypadStateReliable(EVT_MANUAL_ON);
  }
}
...
void updateLogic()
{
  ...
  {
    unsigned long totalRunMs = totalAltSessionMs();
